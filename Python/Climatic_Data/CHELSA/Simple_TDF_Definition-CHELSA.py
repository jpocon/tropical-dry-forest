...
def mon_100(mon100):
    ...
    with r.open(mon100, "r+") as dset:
        arr = dset.read()
        arr[arr == -32767] = 101 # nodata val
        arr[(arr > 0) & (arr <= 100)] = 1
        arr[arr > 100] = -32767 # set all vals > 100 to nodata val
        dset.write(arr)
...
for fp in tile100:
    ...
    dset = r.open(fp, "r+")
    arr = dset.read()
    arr[arr == -32767] = 0 # change nodata vals to 0
    arr = arr.astype("uint8")
...
ex100 = gdal.Open(mon + "/CHELSA_prec_01_V1.2_land_mon100_tropics_compressed.tif", gdal.GA_ReadOnly)
# we are only interested in the first band of the .tif (the precip data)
b100 = ex100.GetRasterBand(1)
# filename for output file
out100 = prec + "/ch-monthly_dry.tif"
...
for file in os.listdir(bio):
    fname=os.fsdecode(file)
    if fname.endswith('CHELSA_bio10_12.tif'):
...
for file in os.listdir(prec):
    fname=os.fsdecode(file)
    if fname.endswith('ch-monthly_dry.tif'):
        path = os.path.join(prec, fname)
...
for file in os.listdir(bio):
    fname = os.fsdecode(file)
    if fname.endswith('CHELSA_bio10_06.tif'):
        path = os.path.join(bio, fname)
...
for fp in tile_simple: # for each file name in the re generated list
    ...
    arr = fname.read()
    arr[arr==-32768] = 0 # read in the array and set no data vals to 0
